...
with app.app_context():
    ...
    if db.session.query(User).filter_by(username='Yasoob').count() < 1:
        db.session.add(User(
          username='Yasoob',
          password=guard.hash_password('yas'),
          roles='admin'
            ))
    if db.session.query(User).filter_by(username='a').count()   <1 :
         db.session.add(User(
            username='a',
            password=guard.hash_password('a'),
            roles='admin'
    ))
    if db.session.query(User).filter_by(username='jew').count()   <1 :
         db.session.add(User(
            username='jew',
            password=guard.hash_password('jew'),
            roles='admin'
    ))
    db.session.commit()
...
def register():
    ...
    db.session.commit()
    sec = {'message': 'successfully sent registration email to user {}'.format(
        new_user.username
    )}
    return (flask.jsonify(sec), 201)
